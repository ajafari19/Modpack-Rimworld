		...
		public override bool MoreChecks(Pawn pawn, Thing t, bool forced = false)
		{
			...
			if (!pawn.IsDesignatedHero())
				if (!RJWSettings.WildMode)
				{
					...
					if (!xxx.can_rape(pawn) || !xxx.can_get_raped(target))
					{
						if (RJWSettings.DevMode) JobFailReason.Is("cannot rape target (vulnerability too low)");
						return false;
					}
					//fail for:
					//satisfied pawns
					...
					//Log.Message("[RJW]WorkGiver_RapeCP::" + xxx.would_fuck(pawn, target));
